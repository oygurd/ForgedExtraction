Forged Extraction

A new modpack I am working on, its main purpose is to be used for an official server for all players to enjoy and play, its an extraction shooter with rpg elements as of now


**ALL MODS CURRENTLY IN THIS MODPACK**

-Embeddium
-Cloth Config API (Fabric/Forge/NeoForge)
-Bookshelf
-Sophisticated Core
-Just Enough Items (JEI)
-Counter Strike Grenades
-More Enchantment Info
-Cull Leaves
-Mouse Tweaks
-JamLib
-KotlinLangForge
-Storage Drawers
-AppleSkin
-Dynamic FPS
-Lios Player Animator Api
-Curios API (Forge/NeoForge)
-EMI Enchants
-More Ores More Gems (Java Edition)
-RightClickHarvest
-VIP Capes
-GuiJS
-Fast Leaf Decay
-Slot Cycler
-Configured
-Farmer's Delight
-Canary
-Sophisticated Backpacks
-Classic Bars
-Unfinished Weapons
-Not Enough Recipe Book [NERB]
-KubeJS
-Immersive Vehicles (MTS/IV) - Official Content Pack - [OCP] - Planes, Cars, Helicopters & More!
-Diet (Fabric/Forge/Quilt)
-Kotlin for Forge
-EMI Ores
-AttributeFix
-Architectury API
-EMIffect (Status Effects EMI Plugin)
-Clumps
-Searchables
-FastBoot
-Rhino
-Unfinished Attachments
-Dank Storage
-Super Chat : Moderation and practice
-Corpse
-Smooth Boot (Reloaded)
-Waystones
-Entity Culling Fabric/Forge
-Construction Wand
-JEIOres
-AllTheCompressed
-CB Multipart
-Copper Age Backport
-Preloading Tricks
-CucumberJS
-AnimationJS
-Elytra Slot
-Inventory Sorter
-EMI professions (EMIP)
-FastSuite
-Tips
-ShatterLib | OctoLib
-Jade Addons (Neo/Forge)
-Simple Voice Chat
-BadOptimizations
-Hyperslide - a movement mod
-Just Enough Professions (JEP)
-Enchantment Descriptions
-GeckoLib
-ModernFix
-TaCZ JS
-SuperMartijn642's Config Lib
-CodeChicken Lib 1.8.+
-Server Monitor
-Jade 🔍
-FTB Library (Forge)
-Mantle
-Controlling
-Cyclops Core
-Photon
-Crafting Tweaks
-FTB Essentials (Forge & Fabric)
--Durability Tooltip
-Apothic Attributes
-Ponder for KubeJS
-Immersive Optimization
-Oculus
-Cucumber Library
-Server Performance - Smooth Chunk Save[Forge/Fabric]
-[TaCZ] Timeless and Classics Zero Guns
-FastWorkbench
-ImmediatelyFast
-MidnightLib
-Distant Horizons: A Level of Detail mod
-TxniLib
-MmmMmmMmmMmm (Target Dummy)
-FerriteCore ((Neo)Forge)
-Max Health Fix
-Redirected
-Balm
-[NTGL] NukaTeam's Gun Lib
-Caelus API
-Xaero's World Map
-Placebo
-Petrolpark Library
-Cupboard
-Accelerated Rendering
-Immersive Vehicles (Formerly Transport Simulator)
-FTB Ultimine (Forge)
-Xaero's Minimap
-AllTheLeaks (Memory Leak Fix)
-Packet Fixer
-Common Capabilities
-Economy+ (Economy Plus)
-Fisherman's Trap [Neo/Fabric]
-Cardiac
-Autochef's Delight
-Crash Utilities
-Puzzles Lib
-TACZ Turrets
-KubeJS GUI
-LootSelect
-Starlight (Forge)
-More Vanilla Enchantments Levels (MVEL)
-EMI
-Sophisticated Storage
-Easy Anvils
-FastFurnace
-Accelerated Recoiling
-Json Things
-playerAnimator
-Just Enough Resources (JER)
-Polymorph (Fabric/Forge/Quilt)
-Framework
-Aquatic Torches
-Moonlight Lib
-Potion Combiner [Potion Blending Combiner]
-Ritchie's Projectile Library
-BetterF3
-Crash Assistant
